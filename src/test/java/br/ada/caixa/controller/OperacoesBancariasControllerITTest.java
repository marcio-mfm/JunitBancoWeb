...
//                .cliente(clienteRepository.findByDocumento(cliente2.getDocumento()).get())
    ...
    void sacarTest() {
        ...
        //when
        var response = restTemplate.<Void>postForEntity(url + "/sacar", saqueRequestDto, Void.class);
    ...
    void transferenciaTest() {
        ...
        //when
        var response = restTemplate.<Void>postForEntity(url + "/tranferir", transferenciaRequestDto, Void.class);

        //then
        assertEquals(HttpStatus.OK, response.getStatusCode());
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
        assertThat(BigDecimal.ZERO.compareTo(contaRepository.findByNumero(numeroConta1).get().getSaldo())).isZero();
        assertThat(valor.compareTo(contaRepository.findByNumero(numeroConta2).get().getSaldo())).isZero();
        assertEquals(0, valor.compareTo(contaRepository.findByNumero(numeroConta1).get().getSaldo()));
